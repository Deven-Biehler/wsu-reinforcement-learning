...
class LFAAgent:
    ...
    def select_action(self, state_game, state_piece):
        ...
        if self.rng.random() < self.epsilon:
            action_pos = self.rng.integers(5)
            action_rot = self.rng.integers(4)
        else:
          Q_vector = []
          for pos in range(5):
            for rot in range(4):
              Q_vector.append(self.phi(state_game, state_piece, pos, rot) @ self.theta)
    ...
    def train(self, pre_state_game, pre_state_piece, pre_action_pos, pre_action_rot, pre_reward,
        ...
        """
        ### BEGIN SOLUTION
        Q_pre = np.dot(self.phi(pre_state_game, pre_state_piece, pre_action_pos, pre_action_rot), self.theta)
        Q_cur = np.dot(self.phi(cur_state_game, cur_state_piece, cur_action_pos, cur_action_rot), self.theta)
        new_Q = (1-self.learning_rate) * Q_pre + self.learning_rate * (pre_reward + self.gamma * Q_cur)
        self.theta += self.learning_rate * (pre_reward + self.gamma * Q_cur - Q_pre) * self.phi(pre_state_game, pre_state_piece, pre_action_pos, pre_action_rot)
        ### END SOLUTION
